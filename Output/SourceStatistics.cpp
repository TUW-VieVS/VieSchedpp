...
void SourceStatistics::writeFile(Network &network, std::vector<Source> &sources,
                                 const std::vector<Scan> &scans, const boost::property_tree::ptree &xml) {
    ...
    if (tmp0.is_initialized()) {
        ...
        unordered_map<std::string, std::vector<std::string>> group_source =
                readGroups(PARA_source, sources);
        ...
        for (const auto &group : group_source) {
            ...
            for (const auto &src : sources) {
                ...
                if (find(group.second.begin(), group.second.end(), src.getName()) != group.second.end()) {
                    ...
                    of << boost::format(
                            "%-8s  #scans: %4d;  #obs: %4d;  weight: %6.2f; min repeat time: %5.2f [h];  visible: %5.2f [h]; ") %
                          src.getName() %
                          scanStartTime[srcid].size() %
                          accumulate(nobs[srcid].begin(), nobs[srcid].end(), 0) %
                          weight[srcid] %
                          minRepeat[srcid] %
                          minVisible[srcid];
            ...
            for (const auto &src : sources) {
                ...
                of << "| ";
                for (auto i : visibleTimes[srcid]) {
                    of << " [ " << TimeSystem::time2timeOfDay(i.first) << " - " << TimeSystem::time2timeOfDay(i.second)
                       << " ] ";
                }
                of << "\n";
