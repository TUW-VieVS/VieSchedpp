...
void Ast::stationParameters(const Station &station, const std::shared_ptr<const ObservingMode> &obsModes) {
    ...
    string tmp;
    tmp  = (boost::format("%7.1f %7.1f %7.1f %7.1f  deg")  %(cableWrap.getWLow()*rad2deg) %(cableWrap.getWUp()*rad2deg) %(cableWrap.getCLow()*rad2deg) %(cableWrap.getCUp()*rad2deg)).str();
    of << boost::format("  %-26s %-6s    %-36s  Axis : %s\n") %"1st_axis_range:" %staName %tmp %ax1;
    tmp = (boost::format("%7.1f %7.1f  deg")  %((cableWrap.getAxis2Low())) %(cableWrap.getAxis2Up()*rad2deg)).str();
    ...
    for(unsigned long i= 0; i<scans.size(); ++i){
        ...
        for (unsigned long staid = 0; staid < stations.size(); ++staid){
            ...
            if(!opt.is_initialized()){
                ...
                // ##### Preob command #####
                of << boost::format("    %-9s %-8s  %s %s  Scan: %-9s  Source: %-8s  Duration: %6.1f  Proc_name: %s\n")
                      %"Preob:"
                      %station.getName()
                      %TimeSystem::time2string_ast(times.getPreobTime(idx, Timestamp::start))
                      %TimeSystem::time2string_ast(times.getPreobTime(idx, Timestamp::end))
                      %name
                      %sourceName
                      %times.getPreobDuration(idx)
                      %"dummy";

                // ##### Record command #####
                of << boost::format("    %-9s %-8s  %s %s  Scan: %-9s  Source: %-8s  Duration: %6.1f  Elev: %5.2f Azim: %7.2f  Hour_angle: %6.2f\n")
                      %"Record:"
                      %station.getName()
                      ...
                      %name
                      %sourceName
                      %times.getObservingDuration(idx)
                      %(pv.getEl()*rad2deg)
                      %(util::wrap2pi(pv.getAz())*rad2deg)
                      %(pv.getHa()*rad2deg);
