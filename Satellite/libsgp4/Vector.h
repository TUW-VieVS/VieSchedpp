...
struct Vector {
   public:
     ...
     */
    Vector operator-( const Vector& v ) { return Vector( x - v.x, y - v.y, z - v.z, 0.0 ); }
     ...
     * @returns dot product
     */
    double Dot( const Vector& vec ) const { return ( x * vec.x ) + ( y * vec.y ) + ( z * vec.z ); }
...
inline std::ostream& operator<<( std::ostream& strm, const Vector& v ) { return strm << v.ToString(); }
