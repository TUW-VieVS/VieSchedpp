...
void SGP4::Initialise() {
    ...
    common_consts_.xnodot = xhdot1 + ( 0.5 * temp2 * ( 4.0 - 19.0 * theta2 ) + 2.0 * temp3 * ( 3.0 - 7.0 * theta2 ) ) *
    ...
    if ( use_deep_space_ ) {
        ...
        if ( elements_.Eccentricity() > 1.0e-4 ) {
            c3 = coef * tsi * kA3OVK2 * elements_.RecoveredMeanMotion() * kAE * common_consts_.sinio /
                 elements_.Eccentricity();
        }

        nearspace_consts_.c5 = 2.0 * coef1 * elements_.RecoveredSemiMajorAxis() * betao2 *
        ...
        if ( !use_simple_model_ ) {
            ...
            nearspace_consts_.t4cof = 0.25 * ( 3.0 * nearspace_consts_.d3 +
                                               common_consts_.c1 * ( 12.0 * nearspace_consts_.d2 + 10.0 * c1sq ) );
...
Eci SGP4::FindPosition( const DateTime& dt ) const { return FindPosition( ( dt - elements_.Epoch() ).TotalMinutes() ); }
...
Eci SGP4::FindPositionSDP4( double tsince ) const {
    ...
    DeepSpaceSecular( tsince, elements_, common_consts_, deepspace_consts_, integrator_params_, xmdf, omgadf, xnode, em,
                      xinc, xn );
    ...
    RecomputeConstants( xinc, perturbed_sinio, perturbed_cosio, perturbed_x3thm1, perturbed_x1mth2, perturbed_x7thm1,
                        perturbed_xlcof, perturbed_aycof );

    /*
     * using calculated values, find position and velocity
     */
    return CalculateFinalPositionVelocity( elements_.Epoch().AddMinutes( tsince ), e, a, omega, xl, xnode, xinc,
                                           perturbed_xlcof, perturbed_aycof, perturbed_x3thm1, perturbed_x1mth2,
                                           perturbed_x7thm1, perturbed_cosio, perturbed_sinio );
}

void SGP4::RecomputeConstants( const double xinc, double& sinio, double& cosio, double& x3thm1, double& x1mth2,
...
Eci SGP4::FindPositionSGP4( double tsince ) const {
    ...
    if ( !use_simple_model_ ) {
        ...
        templ +=
            nearspace_consts_.t3cof * tcube + tfour * ( nearspace_consts_.t4cof + tsince * nearspace_consts_.t5cof );
    }
     ...
     */
    return CalculateFinalPositionVelocity( elements_.Epoch().AddMinutes( tsince ), e, a, omega, xl, xnode, xinc,
                                           common_consts_.xlcof, common_consts_.aycof, common_consts_.x3thm1,
                                           common_consts_.x1mth2, common_consts_.x7thm1, common_consts_.cosio,
                                           common_consts_.sinio );
}

Eci SGP4::CalculateFinalPositionVelocity( const DateTime& dt, const double e, const double a, const double omega,
                                          const double xl, const double xnode, const double xinc, const double xlcof,
                                          const double aycof, const double x3thm1, const double x1mth2,
                                          const double x7thm1, const double cosio, const double sinio ) {
...
void SGP4::DeepSpaceInitialise( const double eosq, const double sinio, const double cosio, const double betao,
    ...
    for ( int cnt = 0; cnt < 2; cnt++ ) {
        ...
        const double z12 =
            -6.0 * ( a1 * a6 + a3 * a5 ) + eosq * ( -24. * ( x2 * x7 + x1 * x8 ) - 6. * ( x3 * x6 + x4 * x5 ) );
        const double z13 = -6.0 * a3 * a6 + eosq * ( -24. * x2 * x8 - 6. * x4 * x6 );
        const double z21 = 6.0 * a2 * a5 + eosq * ( 24. * x1 * x5 - 6. * x3 * x7 );
        const double z22 =
            6.0 * ( a4 * a5 + a2 * a6 ) + eosq * ( 24. * ( x2 * x5 + x1 * x6 ) - 6. * ( x4 * x7 + x3 * x8 ) );
    ...
    if ( elements_.RecoveredMeanMotion() < 0.0052359877 && elements_.RecoveredMeanMotion() > 0.0034906585 ) {
        ...
        deepspace_consts_.xlamo =
            Util::WrapTwoPI( elements_.MeanAnomoly() + elements_.AscendingNode() + elements_.AscendingNode() -
                             deepspace_consts_.gsto - deepspace_consts_.gsto );
        bfact = xmdot + xnodot + xnodot - kTHDT - kTHDT + deepspace_consts_.ssl + deepspace_consts_.ssh +
 ...
 */
void SGP4::DeepSpacePeriodics( const double tsince, const DeepSpaceConstants& ds_constants, double& em, double& xinc,
...
void SGP4::DeepSpaceSecular( const double tsince, const OrbitalElements& elements, const CommonConstants& c_constants,
                             const DeepSpaceConstants& ds_constants, IntegratorParams& integ_params, double& xll,
    ...
    if ( ds_constants.shape != DeepSpaceConstants::NONE ) {
         ...
         */
        if ( fabs( tsince ) < STEP || tsince * integ_params.atime <= 0.0 ||
            ...
            if ( ds_constants.shape == DeepSpaceConstants::SYNCHRONOUS ) {
                ...
                xnddt = ds_constants.d2201 * cos( x2omi + integ_params.xli - G22 ) +
                        ...
                        2.0 * ( ds_constants.d4410 * cos( x2omi + x2li - G44 ) +
                                ds_constants.d4422 * cos( x2li - G44 ) + ds_constants.d5421 * cos( xomi + x2li - G54 ) +
