    ...
    if ( !fid.is_open() ) {
...
#else
        cout << "[error] unable to open " << filename_ << " file";
#endif
        return;
    } else {
        string line;
        ...
        while ( getline( fid, line ) ) {
            if ( line.find( scanName ) != line.npos ) {
                ...
                if ( thisScan.slewStart.is_initialized() && thisScan.slewEnd.is_initialized() &&
                     thisScan.recordOn.is_initialized() && thisScan.recordOff.is_initialized() &&
                     thisScan.preobStart.is_initialized() && thisScan.preobEnd.is_initialized() ) {
                    ...
                    if ( firstScan ) {
                        ...
                        thisScan.realSlewTime =
                            diff_slew.total_seconds() + static_cast<double>( diff_slew.fractional_seconds() ) / 1.0e6;

                        boost::posix_time::time_duration diff_preob = *thisScan.preobEnd - *thisScan.preobStart;
                        thisScan.realPreobTime =
                            diff_preob.total_seconds() + static_cast<double>( diff_preob.fractional_seconds() ) / 1.0e6;

                        boost::posix_time::time_duration diff_idle = *thisScan.preobStart - *thisScan.slewEnd;
                        thisScan.realIdleTime =
                            diff_idle.total_seconds() + static_cast<double>( diff_idle.fractional_seconds() ) / 1.0e6;
                    }

                    boost::posix_time::time_duration diff_record = *thisScan.recordOff - *thisScan.recordOn;
                    thisScan.realScanTime =
                        diff_record.total_seconds() + static_cast<double>( diff_record.fractional_seconds() ) / 1.0e6;
...
void LogParser::output( const std::string &outfile ) {
    ...
    if ( addedScheduledTimes_ ) {
        of << "*                            slew     slew     slew    preob    preob    preob     scan     scan     "
              "scan     idle     idle     idle\n"
           << "*     scan     source  E      log    sched      diff     log    sched     diff      log    sched     "
              "diff      log    sched     diff\n";
    } else {
    ...
    for ( const auto &any : logScans_ ) {
        of << boost::format( "%10s, %9s, " ) % any.scanName % any.sourceName;
        if ( any.error ) {
            of << boost::format( "1, " );
        } else {
            of << boost::format( "0, " );
            ...
            if ( addedScheduledTimes_ ) {
                of << boost::format( "%7d, %7.2f, " ) % any.scheduledSlewTime %
                          ( any.realSlewTime - any.scheduledSlewTime );
            }

            of << boost::format( "%7.2f, " ) % any.realPreobTime;
            if ( addedScheduledTimes_ ) {
                of << boost::format( "%7d, %7.2f, " ) % any.scheduledPreobTime %
                          ( any.realPreobTime - any.scheduledPreobTime );
            }

            of << boost::format( "%7.2f, " ) % any.realScanTime;
            if ( addedScheduledTimes_ ) {
                of << boost::format( "%7d, %7.2f, " ) % any.scheduledScanTime %
                          ( any.realScanTime - any.scheduledScanTime );
            }

            of << boost::format( "%7.2f, " ) % any.realIdleTime;
            if ( addedScheduledTimes_ ) {
                of << boost::format( "%7d, %7.2f, " ) % any.scheduledIdleTime %
                          ( any.realIdleTime - any.scheduledIdleTime );
            }
        }
        of << "\n";
    }
