   ...
   public:
    ...
    enum {
        ...
        TrOptions = RowsAtCompileTime == 1
                        ...
                        : ColsAtCompileTime == 1 ? ( MatrixType::Options | RowMajor ) : MatrixType::Options
    };
...
struct svd_precondition_2x2_block_to_be_real<MatrixType, QRPreconditioner, true> {
    ...
    static bool run( typename SVD::WorkMatrixType& work_matrix, SVD& svd, Index p, Index q, RealScalar& maxDiagEntry ) {
        ...
        maxDiagEntry = numext::maxi<RealScalar>(
            maxDiagEntry,
            numext::maxi<RealScalar>( abs( work_matrix.coeff( p, p ) ), abs( work_matrix.coeff( q, q ) ) ) );
            ...
            for ( Index q = 0; q < p; ++q ) {
                ...
                if ( abs( m_workMatrix.coeff( p, q ) ) > threshold || abs( m_workMatrix.coeff( q, p ) ) > threshold ) {
                    ...
                    if ( internal::svd_precondition_2x2_block_to_be_real<MatrixType, QRPreconditioner>::run(
                        ...
                        maxDiagEntry = numext::maxi<RealScalar>(
                            maxDiagEntry, numext::maxi<RealScalar>( abs( m_workMatrix.coeff( p, p ) ),
