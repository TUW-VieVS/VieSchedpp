 ...
 * support AVX yet... One might also wonder why in the EvenAligned case we perform unaligned loads instead of using the
 * aligned-loads plus re-alignment strategy as in the FirstAligned case. The reason is that we observed that unaligned
 * loads on a 8 byte boundary are not too slow compared to unaligned loads on a 4 byte boundary.
 *
 */
...
template <typename Index, typename LhsScalar, typename LhsMapper, bool ConjugateLhs, typename RhsScalar,
          typename RhsMapper, bool ConjugateRhs, int Version>
EIGEN_DONT_INLINE void general_matrix_vector_product<Index, LhsScalar, LhsMapper, ColMajor, ConjugateLhs, RhsScalar,
...
template <typename Index, typename LhsScalar, typename LhsMapper, bool ConjugateLhs, typename RhsScalar,
          typename RhsMapper, bool ConjugateRhs, int Version>
EIGEN_DONT_INLINE void general_matrix_vector_product<Index, LhsScalar, LhsMapper, RowMajor, ConjugateLhs, RhsScalar,
                                                     RhsMapper, ConjugateRhs, Version>::run( Index rows, Index cols,
                                                                                             const LhsMapper& lhs,
                                                                                             const RhsMapper& rhs,
                                                                                             ResScalar* res,
...
#define _EIGEN_ACCUMULATE_PACKETS( Alignment0, Alignment13, Alignment2 )                  \
    ...
    if ( ( sizeof( LhsScalar ) != sizeof( RhsScalar ) ) || ( lhsAlignmentOffset < 0 ) ||
         ( lhsAlignmentOffset == depth ) || ( rhsAlignmentOffset < 0 ) || ( rhsAlignmentOffset == rows ) ) {
        alignedSize = 0;
        alignedStart = 0;
        alignmentPattern = NoneAligned;
    } else if ( LhsPacketSize > 4 ) {
                    ...
                    case FirstAligned: {
                        ...
                        if ( peels > 1 ) {
                            ...
                            for ( ; j < peeledSize; j += peels * RhsPacketSize ) {
                                ...
                                ptmp0 =
                                    pcj.pmadd( lhs0.template load<LhsPacket, Aligned>( j + LhsPacketSize ), b, ptmp0 );
