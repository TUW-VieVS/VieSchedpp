...
boost::property_tree::ptree ParameterSettings::parameterStation2ptree( const string &name,
    ...
    if ( PARA.maxNumberOfScansDist.is_initialized() ) {
        parameters.add( "parameters.maxNumberOfScansDist", PARA.maxNumberOfScansDist );
    }
...
std::pair<string, ParameterSettings::ParametersStations> ParameterSettings::ptree2parameterStation(
    ...
    for ( auto &it : ptree ) {
        ...
        if ( paraName == "<xmlattr>" ) {
            ...
            para.maxNumberOfScansDist = it.second.get_value<unsigned int>();
...
void ParameterSettings::satelliteAvoidance( const boost::property_tree::ptree &rules ) {
